...
void RBVisualizer::setParameterCode(std::string parameterCode){
    _parameterCode = parameterCode;
}

void RBVisualizer::setTimeStep(int timeStep){
    _timeStep = timeStep;
}
...
void RBVisualizer::readParameterMinMax(){
    ...
    std::string fileNameMax = _filePath +folder+ "RBConvection_" +
      std::to_string(_timeMax) + ".vtk";
    std::string fileNameMin = _filePath + folder+"RBConvection_" +
...
#endif
    ifstream currentFileMax(fileNameMax);
    if (!currentFileMax.is_open()) {
        cout << "File is not opened." << endl;
        throw fileNotExistError();
    }
    ifstream currentFileMin(fileNameMin);
    if (!currentFileMin.is_open()) {
        cout << "File is not opened." << endl;
        throw fileNotExistError();
    }
#ifdef ERROR
    std::cout << "Done checking the file..." << std::endl;
#endif
 ...
 #pragma omp parallel for ordered schedule(guided) num_threads(2)
  for(int fileIndex = _timeStep; fileIndex <= _timeMax; fileIndex += _timeStep){
      ...
      int currentVectorIndex = fileIndex / _timeStep - 1;
        ...
        //gui.setStatusText("loading files...");
      /////    Data manipulation    /////
      std::string fileName = _filePath + folder+"RBConvection_" +
...
#ifdef ERROR
    std::cout << "Set up lookupTables..." << std::endl;
#endif
      // Set up lookupTables
...
#ifdef ERROR
    std::cout << "Set up the color schemes..." << std::endl;
#endif
...
#ifdef ERROR
    std::cout << "Set up the plane properties..." << std::endl;
#endif
...
#ifdef ERROR
    std::cout << "Set up the mapper..." << std::endl;
#endif
